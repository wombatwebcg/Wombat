...
namespace Wombat.Network.Socket
{
    public class TcpSocketClient
    {
       ...
       public static TcpSocketClient UseDispatcherModel(TcpSocketClient client, ITcpSocketClientEventDispatcher dispatcher)
        {
            _dispatcher = dispatcher;
            return client;
        }

        public static TcpSocketClient UseFrameBuilder(TcpSocketClient client, IFrameBuilder frameBuilder)
        {
            _frameBuilder = frameBuilder;
            return client;
        }

        public static TcpSocketClient UsgLogger(TcpSocketClient client, Logger log)
        {
            _logger = log;
            return client;
        }
        ...
        public async Task ConnectAsync()
        {
            ...
            try
            {
               ...
               await _dispatcher?.OnServerConnecting(this);
                ...
                try
                {
                    ...
                    while (State == SocketConnectionState.Connected)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (_frameBuilder != null)
                            {
                                if (_frameBuilder.Decoder.TryDecodeFrame(_receiveBuffer.Array,
                                    _receiveBuffer.Offset + consumedLength, _receiveBufferOffset - consumedLength,
                            ...
                            else
                            {
                                try
                                {
                                    await _dispatcher?.OnServerDataReceived(this, _receiveBuffer.Array
                                        , _receiveBuffer.Offset, _receiveBuffer.Count - _receiveBufferOffset);
                                }
                                catch (Exception ex) // catch all exceptions from out-side
                                {
                                    await HandleUserSideError(ex);
        ...
        private async Task CloseAsync(bool shallNotifyUserSide)
        {
            ...
            await _dispatcher?.OnServerDisconnecting(this);
            Shutdown();
