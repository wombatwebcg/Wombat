...
namespace Wombat.Sockets
{
    public sealed class TcpSocketSession
    {
        ...
        private static  ILogger<TcpSocketSession>  Logger => (ILogger<TcpSocketSession>)LogHelper.Log;
        ...
        private bool CloseIfShould(Exception ex)
        {
                ...
                )
            {
                if (ex is SocketException)
                    Logger?.LogError(string.Format("Session [{0}] exception occurred, [{1}].", this, ex.Message), ex);
