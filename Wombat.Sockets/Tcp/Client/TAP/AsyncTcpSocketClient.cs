...
namespace Wombat.Sockets
{
    public class AsyncTcpSocketClient
    {
        ...
        private static ILogger<AsyncTcpSocketClient> Logger => new SerilogLoggerFactory(LogHelper.Log).CreateLogger<AsyncTcpSocketClient>();
        ...
        public async Task Connect()
        {
            ...
            try
            {
                ...
                Logger?.LogDebug("Connected to server [{0}] with dispatcher [{1}] on [{2}].",
                    this.RemoteEndPoint,
        ...
        private async Task Close(bool shallNotifyUserSide)
        {
            ...
            {
                Logger?.LogDebug($"Disconnected from server [{this.RemoteEndPoint}] " +
        ...
        {
            Logger?.LogError($"Client [{this}] error occurred in user side [{ex.Message}].");
